...
sapply(1:length(spatial_samples), function(i) {
...
p1 = SpatialFeaturePlot(keep, features = "log10_nCount") +
  theme(legend.text = element_text(size = 5), alpha = c(0.1, 1)) + 
  ggtitle(paste0(names(spatial_samples)[i] , ": filtered"))

p2 = SpatialFeaturePlot( spatial_samples[[i]], features = "log10_nCount") +
  theme(legend.text = element_text(size = 5), alpha = c(0.1, 1)) + 
  ggtitle(paste0(names(spatial_samples)[i] , ": raw"))
...
```{r filter_plots, eval =F, fig.width=9}

#SpatialDimPlot(spatial_samples[[1]])
sapply(1:length(spatial_samples), function(i) {
